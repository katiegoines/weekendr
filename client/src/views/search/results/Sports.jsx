...
class Sports extends React.Component {
    ...
    request() {
        ...
        axios({method: 'get', url: `/api/search/eventful?location=${this.props.search}&category=sports&dateRange=${dates}`})
        .then((res) => { 
            if(res.data.fullType === "rest-call.response-filters.unhandled-status") {
                throw new Error("error")
            } else {
                console.log(res.data.sports.event)
                this.setState({
                    results: {
                        list: res.data.sports.event, 
                        head: "Sports"
                    }
                })
            }
